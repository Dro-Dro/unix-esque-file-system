...
i32 fsRead(i32 fd, i32 numb, void* buf) {
  ...
  i32 inum = bfsFdToInum(fd);
  i32 curs = fsTell(fd);
  i32 fbnStart = curs / BYTESPERBLOCK;
  i32 fbnEnd = (curs + numb) / BYTESPERBLOCK;
  i32 Dbn = bfsFbnToDbn(inum, fbnStart);
  i8 bioBuf[2000];
  bioRead(Dbn, bioBuf);
  memcpy(buf, bioBuf, numb);
  // for (int i = 0; i < BYTESPERBLOCK; i++) {
  //   bytesRead++;
  //   int currBlock = (bytesRead / BYTESPERBLOCK) * BYTESPERBLOCK;
  //   (i8*)buf[currBlock + i] = bioBuf[i];
  // }
...
i32 fsWrite(i32 fd, i32 numb, void* buf) {
  ...
  i32 curs = fsTell(fd);
  i32 fbnStart = curs / BYTESPERBLOCK;
  i32 currCurs = curs - (fbnStart * BYTESPERBLOCK);
  i32 Dbn = bfsFbnToDbn(inum, fbnStart);
  i8 bioBuf[2000];
  memset(bioBuf, fbnStart, 2000);
  memcpy(bioBuf + currCurs, buf, numb);
  bioWrite(Dbn, bioBuf);
  fsSeek(fd, numb, SEEK_CUR);
